!
! Subroutine compute_gdihed: Computes the dihedral angle energy gradient
! 
! L. Martinez, Dez 03, 2013
! Institute of Chemistry - State University of Campinas - Brazil
!

subroutine compute_gdihed(x,g)
...
! Should I change this derivative by the one computed from
! the atan2 ? The current one is correct, but apparently it is
! less stable. This is the computation of the angle which is
! currently done in function dihedangle
!    v1(1) = x(jx) - x(ix)
!    v1(2) = x(jy) - x(iy)
!    v1(3) = x(jz) - x(iz)
!
...
!    phi = atan2(sinphi,cosphi)
    ...
    dphi_dcosphi = pnorm_inv / dsqrt( 1.d0 - cosphi**2 )
    ...
    do j = 0, idihed(i,6) - 1
      ...
      de_dphi = fdihed(k,1)*fdihed(k,2)*dsin(fdihed(k,2)*phi-fdihed(k,3))
