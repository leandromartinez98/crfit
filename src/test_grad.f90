...
subroutine test_grad(n,x,g,computef,computeg)
  ...
  implicit none
  integer ::  n, i, iworst
  double precision :: x(*), g(*), fx, step, gcomp, gbest, eworst, error, steperror, stepbest
  ...
  write(*,*)
  write(*,*) ' Coordinates of 5 first atoms: '
  do i = 1, 5
  ...
  do i = 1, n
    ...
    do while(error.gt.1.d-6.and.step.ge.1.d-20)
      call discret(i,x,gcomp,step,computef)
    ...
    flush(98)
    if ( i <= 9 ) then
      write(*,"(i10,4(tr2,d13.7))") i, g(i), gbest, error, stepbest 
    end if
...
subroutine discret(icomp,x,gcomp,step,computef)
  implicit none
  integer :: icomp
  ...
  save = x(icomp) 
  x(icomp) = save + step 
  call computef(x, fplus) 
  x(icomp) = save - step 
  call computef(x, fminus) 
