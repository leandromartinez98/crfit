module flashsortvars
  integer :: mflash
  integer, allocatable :: indflash(:), lflash(:), indflashback(:)
end module flashsortvars
!
!integer :: mflash
!integer, allocatable :: indflash(:), lflash(:)
!allocate( indflash(nca), lflash(nca))
!mflash = 1 + nca / 10
!call flashsort(scores, nca, lflash, mflash, indflash)
!
...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine flashsort(A, N, L, M, ind, indback)

      double precision a(n), anmin, c1, hold, flash
      integer L(*), ind(*), indback(*), i, n, nmax, m, k, ihold, nmove, j, iflash
!     ============================ CLASS FORMATION ===== 
