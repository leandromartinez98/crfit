...
!                     allocates the vectors containing the the dihedral parameters,
!                     and fill them  
!
...
subroutine readprm(parameter_file,n_parameter_file)

  use force_field
  implicit none
  integer :: ioerr, i, j, k, n_parameter_file, &
             n_bond_lines, n_angle_lines, n_nonbonded_lines, n_impr_lines, n_dihed_lines, &
             dummyi
  integer, allocatable :: nonespecific_dihed(:), nonespecific_impr(:)
  double precision :: dummyr
  character(len=5) :: atom1, atom2, atom3, atom4
  character(len=200) :: parameter_file(*), record, keyword
  character(len=5), allocatable :: bond_chars(:,:), angle_chars(:,:), nonbonded_chars(:,:),&
                                   impr_chars(:,:), dihed_chars(:,:)
  double precision, allocatable :: bond_pars(:,:), angle_pars(:,:), nonbonded_pars(:,:),&
                                   impr_pars(:,:), dihed_pars(:,:), read_urey_bradley(:,:)
  double precision, parameter :: pi = 3.14159265358979323846d0
  logical, allocatable :: has_urey_bradley(:)
  logical :: missing_pars, comment_line

  write(*,*)
  write(*,*) ' Reading parameter files. '

  ! Here we will count, within all parameter files, the number of parameters
